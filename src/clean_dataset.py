...
def get_strategy(letter):
    """Determines the filtering strategy based on the class size."""
...
def clean_dataset(data_dir):
    """
    Executes the automated cleaning heuristics on the specified dataset directory.
    
    Args:
        data_dir (str or Path): The root directory containing the letter subfolders.
        
    Returns:
        dict: A dictionary containing the total filtering statistics.
    """
    root_path = Path(data_dir)
    
    total_stats = {
        "total": 0, "white": 0, "dark": 0, 
        "contrast": 0, "gray": 0, "review": 0, "clean": 0
    }

    for letter in LETTERS:
        folder = root_path / letter
...
if __name__ == "__main__":
    # If run directly as a script, execute on the default path
    clean_dataset("../data/ALPUB_v2/images")